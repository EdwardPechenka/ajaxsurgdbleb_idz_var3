...
<body>
    ...
    function parseAndRenderXml(xmlText) {
        ...
        if (items.length === 0) {
            return '<h3>Медсестри відділення (формат XML)</h3><p>У цьому відділенні немає медсестер.</p>';
        }
    ...
    function renderJson(data) {
        if (data.length === 0) {
            return '<h3>Чергування у зміну (формат JSON)</h3><p>Жодних чергувань на цю зміну не знайдено.</p>';
        }
    ...
    document.getElementById('nurseForm').addEventListener('submit', function(e) {
        ...
        const cacheKey = 'ajax_nurse_' + nurseId;
        
        const cached = localStorage.getItem(cacheKey);
        if (cached) {
            resultDiv.innerHTML = cached + ' <small style="color:green;display:block;margin-top:5px;">(Взято з локального кешу)</small>';
            return;
        }
    ...
    document.getElementById('departmentForm').addEventListener('submit', function(e) {
        ...
        const cacheKey = 'ajax_dept_' + deptId;
        
        const cached = localStorage.getItem(cacheKey);
        if (cached) {
            resultDiv.innerHTML = parseAndRenderXml(cached) + ' <small style="color:green;display:block;margin-top:5px;">(Из локального кэша)</small>';
            return;
        }
        ...
        xhr.onreadystatechange = function() {
            if (xhr.readyState === 4 && xhr.status === 200) {
                localStorage.setItem(cacheKey, xhr.responseText);
                resultDiv.innerHTML = parseAndRenderXml(xhr.responseText);
            }
        };
        xhr.send();
    });
    ...
    document.getElementById('shiftForm').addEventListener('submit', function(e) {
        ...
        const cacheKey = 'ajax_shift_' + shiftValue;
        
        const cached = localStorage.getItem(cacheKey);
        if (cached) {
            const data = JSON.parse(cached);
            resultDiv.innerHTML = renderJson(data) + ' <small style="color:green;display:block;margin-top:5px;">(Из локального кэша)</small>';
            return;
        }
        ...
        fetch(`shift_duties.php?shift=${shiftValue}`)
            ...
            .then(text => {
                ...
                resultDiv.innerHTML = renderJson(data);
            })
            .catch(error => {
